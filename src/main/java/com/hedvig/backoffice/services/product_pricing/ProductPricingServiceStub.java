...
public class ProductPricingServiceStub implements ProductPricingService {
    ...
    public ProductPricingServiceStub(ObjectMapper mapper) {
        ...
        insurances = IntStream.range(0, MemberServiceStub.testMemberIds.length).mapToObj(i -> {
            ...
            InsuranceStatusDTO insurance = new InsuranceStatusDTO(UUID.randomUUID().toString(), memberId,
                    ("Firstname" + memberId), ("Lastname" + memberId), "Street", "City", "ZipCode", 0, (float) 50,
                    safetyIncreasers, "PENDING", insuranceState, RandomUtils.nextInt(0, 9),
                    new BigDecimal(Math.random()), null, true, "BRF", null, null, false, null, false, null);
            ...
            if (insurance.getInsuranceState().equals(states[1])) {
                ...
                LocalTime randomSignedOnLocalTime = LocalTime
                        .ofNanoOfDay(randomSignedOnDate * RandomUtils.nextInt(0, 1000000));
                insurance.setSignedOn(Instant
    ...
    public InsuranceStatusDTO insurance(String memberId, String token) {
        Optional<InsuranceStatusDTO> insurance = insurances.stream().filter(i -> i.getMemberId().equals(memberId))
                .findAny();
    ...
    public void activate(String memberId, InsuranceActivateDTO dto, String token) {
        this.insurance(memberId, token).setInsuranceActiveFrom(
    ...
    public List<InsuranceStatusDTO> search(String state, String query, String token) {
        ...
        return insurances.stream()
                .filter(u -> (StringUtils.isNotBlank(query) && u.getMemberFirstName().contains(query))
